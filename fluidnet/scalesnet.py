...
class ScalesNet(BaseModel):
    def __init__(self, scales: dict, kernel_sizes):
        ...
        for iscale, scale in enumerate(scales.values()):
            ...
            if isinstance(scale[0], list):
                for elem in scale:
                    tmp_list += elem
                    tmp_params += sum([elem[i] * elem[i + 1] * (self.kernel_sizes[iscale]**3) + elem[i] for
                                       i in range(len(elem) - 1)])
                self.width.append(len(tmp_list) - 2)
            else:
                tmp_list = scale
                tmp_params += sum([scale[i] * scale[i + 1] * (self.kernel_sizes[iscale]**3) + scale[i] for
    ...
    def global_prop(self):
        ...
        depth_str = f'Height of the network: {self.depth:d}'
        prop_str = 'Global properties of each scale: \n'
        prop_str += f'{" ":11}|{"RF":^10}|{"depth":^10}|{"nparams":^10}|{"k-size":^10}|'
        for d in range(self.n_scales):
            prop_str += f'\nBranch {d:d}   |{self.receptive_field[d]:10d}|{self.width[d]:10d}|{self.params[d]:10d}|{self.kernel_sizes[d]:10d}|'
