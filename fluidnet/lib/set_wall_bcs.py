...
def setWallBcs(U, flags):
    ...
    obst100 = (flags[idx_b, zero, k, j, i_l].eq(CellType.TypeObstacle).int().__and__(mCont.int())).bool()
    U[:, 0].masked_fill_(obst100, 0)

    # Current cell is an obstacle.
    # The neighbour to the left (i-1,j,k) is fluid.
    # Set u.n = u_solid.n (slip bc) (direction i)
    obs_fluid100 = ((flags[idx_b, zero, k, j, i_l].eq(CellType.TypeFluid).int()
                     ).__and__(cur_obs.int()).__and__(mCont.int())).bool()
    U[:, 0].masked_fill_(obs_fluid100, 0)

    j_l = zero.where((j <= 0), j - 1)
    obst010 = ((flags[idx_b, zero, k, j_l, i].eq(CellType.TypeObstacle)).int().__and__(mCont.int())).bool()
    U[:, 1].masked_fill_(obst010, 0)
    obs_fluid010 = ((flags[idx_b, zero, k, j_l, i].eq(CellType.TypeFluid)
                     ).int().__and__(cur_obs.int()).__and__(mCont.int())).bool()
    U[:, 1].masked_fill_(obs_fluid010, 0)

    if (is3D):
        k_l = zero.where((k <= 0), k - 1)
        obst001 = ((flags[idx_b, zero, k_l, j, i].eq(CellType.TypeObstacle)).int().__and__(mCont.int())).bool()
        U[:, 2].masked_fill_(obst001, 0)

        obs_fluid001 = ((flags[idx_b, zero, k_l, j, i].eq(CellType.TypeFluid)
                         ).int().__and__(cur_obs.int()).__and__(mCont.int())).bool()
...
if __name__ == '__main__':
    U = torch.randint(0, 99, (2, 5, 5)).float()
    flags = torch.randint(1, 2, (5, 5))
    flags[0] = 2
    flags[-1] = 2
    # flags[:, 0] = 2
    # flags[:, -1] = 2
    print(flags)
    print(U)
    U_new = setWallBcs(U.view(1, 2, 1, 5, 5), flags.view(1, 1, 1, 5, 5))
    U_new = U_new.squeeze()
    print(U_new)
