  ...
  for (int k = 0; k <= 2; ++k) {
    for (int l = 0; l <= 2; ++l) {
      ...
      for (int s = 0; s < NUM_IMAGES; ++s) {
        for (int m = 0; m <= 2; ++m) {
          for (int n = 0; n <= 2; ++n) {
            z += WEIGHT[3*(3*(NUM_IMAGES*p+s)+m)+n] * image[s][i+m][j+n];
          }
        }
      }
      z *= x[c][0][i+k][j+l];
    ...
    for (int k = 0; k <= 2; ++k) {
      for (int l = 0; l <= 2; ++l) {
        ...
        for (int s = 0; s < NUM_IMAGES; ++s) {
          for (int m = 0; m <= 2; ++m) {
            for (int n = 0; n <= 2; ++n) {
              z += WEIGHT[3*(3*(NUM_IMAGES*idx_kl+s)+m)+n] * image[s][ii+m][jj+n];
            }
          }
        }
...
std::vector<torch::Tensor> sm_block_cuda_backward(
  ...
  AT_DISPATCH_FLOATING_TYPES(grad_output.type(), "sm_block_cuda_dwdb", ([&] {
    sm_block_cuda_dwdb_fast_kernel<scalar_t><<<dwdb_blocks, threads>>>(
  ...
  AT_DISPATCH_FLOATING_TYPES(grad_output.type(), "sm_block_cuda_dx", ([&] {
    sm_block_cuda_dx_kernel<scalar_t><<<dx_blocks, threads>>>(
        grad_output.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
        image.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
        x.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
