#include <pybind11/pybind11.h>
#include <pybind11/eigen.h>
#include "ICSolver.h"
#include "BinaryIO.h"

namespace py = pybind11;

using IC = ICSolver<PBackend, SBackend>;

PYBIND11_MODULE(pyic_vexcl, m) {
    m.doc() = "python binding for Incomplete Cholesky preconditioned solver VexCL version";
    m.def("solve", [](const SpMat& A, const VXT& b, double tol=1e-4, double atol=1e-10, int max_iters=100) {
        VXT x(b.size());
        x.setZero();
        auto info = IC::Solve(A, x, b, tol, atol, max_iters);
        return std::make_tuple(x, info);
    });
}

